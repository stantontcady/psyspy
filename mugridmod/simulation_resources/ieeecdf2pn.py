...
from microgrid_model import Bus, ConstantApparentPowerModel as PQ, ConstantVoltageMagnitudeRealPowerModel as PV
from microgrid_model import PowerLine, PowerNetwork
        ...
        for line_number, line in enumerate(cdf_file):
            ...
            if title_data is None:
                ...
                if line[0:4] == "-999":
                    ...
                    V = bus_data["V"]
                    theta = bus_data["theta"]
                    shunt_y = (bus_data["shunt_g"], bus_data["shunt_b"])
                    if bus_data["bus_type"] == 3:
                        P = bus_data["gen_p"] - bus_data["load_p"]
                        bus_id = n.add_bus(Bus(model=PV(P=P), V0=V, theta0=theta, shunt_y=shunt_y, name=bus_name), is_slack_bus=True)
                    elif bus_data["bus_type"] == 2:
                        P = bus_data["gen_p"] - bus_data["load_p"]
                        bus_id = n.add_bus(Bus(model=PV(P=P), V0=V, theta0=theta, shunt_y=shunt_y, name=bus_name))
                    elif bus_data["bus_type"] == 0:
                        P = bus_data["load_p"] - bus_data["gen_p"]
                        Q = bus_data["load_q"] - bus_data["gen_q"]
                        bus_id = n.add_bus(Bus(model=PQ(P=P, Q=Q), V0=V, theta0=theta, shunt_y=shunt_y, name=bus_name))
                    else:
                        debug("bus type number %i is unsupported, ignoring bus number %i" % (bus_type, bus_num))
                        bus_id = None
                    if bus_id is not None:
                        bus_id_bus_num_map[bus_data["bus_num"]] = bus_id
                ...
                if line[0:4] == "-999":
                    ...
                    bus_a = n.get_bus_by_id(bus_id_bus_num_map[branch_data["tap_bus_num"]])
                    bus_b = n.get_bus_by_id(bus_id_bus_num_map[branch_data["z_bus_num"]])
                    _ = bus_a.add_to_shunt_impedance(shunt_y=(0, 0.5*branch_data["b"]))
                    _ = bus_b.add_to_shunt_impedance(shunt_y=(0, 0.5*branch_data["b"]))
                    n.connect_buses(bus_a, bus_b, z=(branch_data["r"], branch_data["x"]))
