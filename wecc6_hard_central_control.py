...
from numpy import arange, array, concatenate, empty, zeros, set_printoptions, isclose, sum as asum
...
def integrate(theta0, n, tsim, tstep):
    ...
    t = 0.
    tperturb_start = 0.125
    tperturb_end = 0.15
    tround = 0.1
    solver = RungeKutta45(step_size=tstep)
    theta_array[0, :] = theta0
    global U
    dU = empty((int(tsim/tround)+1, 3))
    dU[0, :] = U[0:3]
    r = 1
    for k in range(1, num_steps):
        if isclose((t % tround), 0) is True:
            gamma = asum(U)/asum(D)
            dU[r, :] = alpha*gamma
            U[0:3] += dU[r, :]
            r += 1
